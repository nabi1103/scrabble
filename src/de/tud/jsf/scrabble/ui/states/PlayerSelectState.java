...
//    	if (!Launch.debug) start.addComponent(new ImageRenderComponent(new Image(DBOX)));  
		...
		DialogueButton start = new DialogueButton("start_button",
				new Vector2f(CENTERX - DISTANCE_BETWEEN_BUTTONS, WINDOW_HEIGHT - 100), "start");
...
//    	if (!Launch.debug) back.addComponent(new ImageRenderComponent(new Image(DBOX)));    
		...
		back = new DialogueButton("back_button", new Vector2f(CENTERX + DISTANCE_BETWEEN_BUTTONS, WINDOW_HEIGHT - 100),
				"back");
...
//		graphic.drawString("Back", back.getPosition().getX() - 15,back.getPosition().getY() - 10);
		for (int i = 0; i < playerBoxes.length; i++) {
			graphic.drawString(playerBoxes[i].getName(), playerBoxes[i].getPosition().getX() - 25,
					playerBoxes[i].getPosition().getY() - 10);
		}
//		graphic.drawString("Players: " + numberOfPlayers, back.getPosition().getX(),back.getPosition().getY() + 70);
		...
		graphic.setColor(new Color(0, 0, 0));
		graphic.drawString(
				"Press the button to set the difficulty\nof trading letters. Current mode is "
						+ (GameplayState.advanced_trade ? "hard\n(With minigame)" : "easy\n(Without minigame)"),
				10, 360);
